---
name: paper-writing
description: "General academic paper writing workflow based on SNL editorial practice. Use for manuscript drafting, section rewriting, rebuttals, and paper polishing when the work is primarily about research-paper prose or LaTeX/Overleaf workflow and no more specialized installed skill is a clearer fit. Prefer academic-paper-skills strategist or composer for explicit planning-versus-composition routing, stats-paper-writing-agent-skills for statistics-heavy manuscripts and reviewer letters, and AI-Research-SKILLs ml-paper-writing for ML or AI venue papers."
---
...
Use this skill as the general manuscript-writing default, but let narrower skills take precedence when the task is clearly one of these cases:

- statistics or JDS-style manuscript work: `stats-paper-writing-agent-skills/skills/stat-writing`
- ML or AI conference paper drafting: `AI-Research-SKILLs/20-ml-paper-writing/ml-paper-writing`
- planning or originality analysis before drafting: `academic-paper-skills/strategist`
- plan-to-prose execution with the academic-paper-skills pack: `academic-paper-skills/composer`

## How This Skill Works

This skill encodes the writing methodology of the [Systems and Networking Lab (SNL)](https://github.com/SNL-UCSB) at UC Santa Barbara, derived from forensic analysis of 6 papers (8 submissions), 7,600+ Overleaf edits, 100+ tex file versions, and 5 peer review processes. See [*The Paper Behind the Paper*](https://sites.cs.ucsb.edu/~arpitgupta/blog/the-paper-behind-the-paper.html) for the full analysis. It works out of the box 鈥?the default rules are calibrated and battle-tested.

### Three Layers

1. **The pipeline (fixed)**: A five-stage writing workflow. Does not change between users or papers.

2. **The voice and editorial rules (defaults provided, customizable)**: Sentence-level style, structural rules, compression patterns, section checklists. These ship with the SNL lab's rules as defaults. Students may customize by editing files in `author_profile/` 鈥?see the README for what to change.
...
- **Gap analysis** 鈫?feeds Brainstorming Phase 1 (Problem Discovery). The gaps the survey identified 鈥?missing quadrants, shared assumptions that break, unexplored combinations 鈥?are the structural limitations that motivate your paper.
- **Writing craft extractions** (Pass 3+) 鈫?feed the Architecture stage and section drafting. The introduction anatomy, evaluation architecture, and design craft you extracted from the best papers in your area are the models for your own paper's structure.
- **Competitive positioning** 鈫?feeds Brainstorming Phase 4. The invariant matrix and dependency graph from synthesis show exactly where your paper sits relative to existing work.

**From the [data-visualization-skill](https://github.com/SNL-UCSB/data-visualization-skill):**
- **Exploration** (`exploration_log.md`) 鈫?feeds Brainstorming Phase 3 (Evaluation Design). The exploration forced you to look at your data from multiple angles before forming hypotheses. The surprises you found 鈥?distributions you didn't expect, subgroups that behaved differently 鈥?shape what claims are defensible and where the real contribution lives.
- **Brainstorm** (`braindump.md`) 鈫?feeds the figure/table plan. Each braindump articulates what question a figure answers, what you expected to see, and what would surprise you. These are the hypotheses your evaluation must validate.
- **Plan + Execute** (`plot_context.md`) 鈫?feeds the Architecture stage's figure/table plan. Each plot_context records intent, variable mappings, plot type rationale, and design decisions 鈥?ready-made entries for the paper's figure plan.
- **Analyze** (WALTER narrations) 鈫?feeds Evaluation Move 4 (Takeaway Synthesis). The WALTER Result 鈥?"what is the takeaway? does it connect back to the hypothesis?" 鈥?is a first draft of the Takeaway paragraph for that experiment cluster.
...
2. Read ALL files in `author_profile/` 鈥?these are the source of truth for editorial rules
...
7. Check for artifacts from sibling skills 鈥?survey paper notes with craft extractions, `exploration_log.md`, `braindump.md`, `plot_context.md`, WALTER narrations. If found, load them as reference material for the relevant pipeline stages

---

## Structured Brainstorming 鈥?The Skill's Centerpiece

**The biggest obstacle for students isn't writing 鈥?it's that their ideas live as unstructured intuitions.** They know something is interesting but can't articulate what or why. The brainstorming process transforms scattered thinking into a precise project context that drives every section of the paper.
...
- **Push back on vague answers.** "It's faster" 鈫?"Faster for whom? By how much? On what workload?" Every answer should be specific enough to appear in the paper.
- **Distinguish structural from quantitative.** "Existing tools aren't accurate enough" is quantitative 鈥?it motivates more experiments. "Existing tools assume stationarity, which fails on bursty data" is structural 鈥?it motivates a new approach. Papers need structural gaps.
...
|---|---|
| `author_profile/editorial_principles.md` | 14 cross-paper principles with evidence (introduction-twice, named-over-vague, what鈫抴hy鈫抯o-what headings, compress-after-expanding, etc.) |
...
| `author_profile/intervention_types.md` | 7 types of advisor interventions 鈥?use this to simulate advisor feedback on drafts |
...
- Zero hedging. "We show" not "We believe." "X reduces Y by 13脳" not "X may help reduce Y."
- Active voice everywhere 鈥?no exceptions. Passive voice obscures agency and weakens prose.
...
- Signpost through claims: section openers may state the section's conclusion ("This section shows that X reduces Y by 13脳") but never use content-free placeholders ("In this section, we describe..."). The test: does the opener tell a skim-reader what the section *concludes*?
- No exclamation marks. No rhetorical questions outside introductions.
- Paragraphs: 4鈥? sentences. Every paragraph does exactly one of: make a claim, present evidence, synthesize a takeaway.
- Headings are claims, not topics. "Event-centric decomposition reduces error 13脳" not "Experimental Results."
...
## Mandatory Style Audit (GATE 鈥?applies to ALL tex edits)

**Before presenting or committing ANY new or modified tex content, Claude MUST run a sentence-level style audit.** This is not optional, not triggered by the user, and not limited to full section drafts 鈥?it applies to every edit, including paragraph-level changes, subsection additions, and overview rewrites.
...
6. **Passive voice**: "accuracy was achieved by X" 鈫?"X achieves". "Experiments were conducted on X" 鈫?"We evaluate on X". Active voice everywhere 鈥?no exceptions.
...
Flag every violation before presenting the draft. Severity levels: CRITICAL (structural 鈥?will cause rejection), MAJOR (visible to reviewers), MINOR (polish-level).
...
|---|---|---|
| Introduction | `section_rhetorical_moves/introduction.md` | Stakes 鈫?Problem Gap 鈫?Key Abstraction 鈫?Design Intuition 鈫?Contributions 鈫?Results Preview |
| Evaluation | `section_rhetorical_moves/evaluation.md` | Setup Anchoring 鈫?Head-to-Head 鈫?Deep Dive 鈫?Takeaway Synthesis 鈫?Ablation 鈫?Robustness |
| Design | `section_rhetorical_moves/design.md` | Abstraction Introduction 鈫?Design Justification 鈫?Component Architecture 鈫?Key Design Decision 鈫?Robustness |
| Related Work | `section_rhetorical_moves/related_work.md` | Category Clustering 鈫?Per-Category Limitation 鈫?Positioning Sentence |
...
### Stage 1: Structured Brainstorming 鈫?Project Context Creation

**Gate**: The user must have a one-sentence identity statement and contribution claims written as results. If they don't, read `brainstorming_guide.md` and walk them through all 6 phases interactively. Don't rush 鈥?this is the most important stage. A vague project context produces a vague paper.
...
**Enforced order**: Draft 0 Introduction 鈫?Evaluation 鈫?Design/Method 鈫?Background 鈫?Related Work 鈫?Final Introduction 鈫?Abstract.

The introduction is written **twice**. This is the most impactful principle in the entire system (Principle 1).

**Draft 0 Introduction** comes first. It is a framing scaffold 鈥?stakes, problem gap, rough contribution claims 鈥?that sets guardrails for the evaluation. Draft 0 clarifies what the paper is *trying* to show. It is explicitly disposable: it probably will not survive to the final version. Writing is a thinking tool, not just a communication tool 鈥?Draft 0 forces the student to externalize their framing before designing experiments.

**The evaluation** comes next, constrained by Draft 0's guardrails. Then Design, Background, and Related Work.

**The final introduction** is rewritten from scratch after the evaluation is complete. It promises exactly what the evidence supports 鈥?no more, no less. Draft 0 is reference material, not a starting point for editing. If the user asks to skip Draft 0 and go straight to evaluation, explain that evaluation without framing guardrails produces experiments that don't build toward a unified argument.

**Per-section scaffolding**: Before writing any section's full prose, write the topic sentences first. Read them in sequence 鈥?they should form a coherent argument on their own. If the topic sentences don't flow, the paragraphs won't either. Fill in the full paragraphs only after the topic-sentence sequence holds together. In LaTeX, a practical technique is to annotate each paragraph with a purpose comment before writing prose:
...
- **Identity stability**: Read the first sentence of every section 鈥?do they tell a coherent story?
- **Flow audit**: Read the last sentence of paragraph N and the first sentence of paragraph N+1, throughout the paper. Does each transition work? Composition 鈥?the logical flow a reader can follow 鈥?is the most important aspect of technical writing. Readers forgive imperfect grammar but cannot follow broken logical progression. Structure produces flow, but structure alone doesn't guarantee it 鈥?the transitions between structural units matter.
...
2. Paragraph merging (multiple examples of same point 鈫?one best example)
3. Generic adjective removal ("significant" 鈫?specific number or delete)
...
Target: 30鈥?0% reduction from first draft. Report character count before and after.
...
After compression and before submission, Claude MUST automatically run these checks using shell commands on the paper's `.tex` and compiled `.pdf` files. Do not ask the student to run them manually 鈥?execute them and report results.

**1. Page count.** Extract page count from the compiled PDF and compare against the venue's limit (from `project_context.md`). Flag whether references/appendices count 鈥?this varies by venue.
```bash
...
```
Flag any PNG/JPG figures 鈥?these should be vector unless they are photographs or screenshots.
...
|-------|--------|---------|
| Page count | 鉁?or 鉁?| N pages (limit: M) |
| Broken refs | 鉁?or 鉁?| List of undefined refs |
| Embedded fonts | 鉁?or 鉁?| List of non-embedded fonts |
| Figure quality | 鉁?or 鉁?| List of raster figures |
| Anonymization | 鉁?or 鉁?| List of leaks found |
| Column balance | 鉁?or 鉁?| Package present/missing |
| LaTeX issues | 鉁?or 鉁?| Count of dangling refs, duplicate labels |
...
4. Identify which stage the paper is in 鈥?enforce introduction-twice ordering (Draft 0 intro 鈫?Evaluation 鈫?Design 鈫?Background 鈫?Related Work 鈫?Final intro 鈫?Abstract)
...
3. Apply in order: framing 鈫?structural rewrite 鈫?background deletion 鈫?evaluation strengthening 鈫?terminology tightening 鈫?claim-first conversion 鈫?compression
...
1. **First readings are precious.** Someone can read your work for the first time only once. Don't send your first draft to everyone simultaneously 鈥?chain your feedback. Send to one reader, incorporate their feedback, then send the revised version to the next reader. Each iteration strengthens the draft before it reaches the next pair of fresh eyes.
...
2. Classify each concern by severity: framing (most dangerous) 鈫?design 鈫?scope 鈫?rigor (most manageable)
3. For each concern: acknowledge 鈫?explain what changed 鈫?point to specific section/figure
...
### "I'm starting a new paper 鈥?where do I begin?"
1. Welcome them. Explain the five-stage pipeline 鈥?especially the introduction-twice principle.
...
4. After brainstorming: write a **Draft 0 introduction** 鈥?a disposable framing scaffold (stakes, problem gap, rough contributions) that sets guardrails for the evaluation. Then point them to `section_rhetorical_moves/evaluation.md` 鈥?they'll write the evaluation next, constrained by Draft 0.
5. Remind them: "Your first draft should be comprehensive 鈥?include everything. Compression comes later. The goal is to get material on paper, not to be concise. Draft 0 of the introduction will probably not survive 鈥?that's the point. It clarifies your thinking."
...
5. Run spec mode 鈥?walk through archetype-specific questions to produce a `figure_spec.md`
6. Run generate mode 鈥?assemble an AI prompt or TikZ code, produce the figure
7. Run critique mode 鈥?check against claims, venue formatting, and design principles
