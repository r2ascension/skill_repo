...
4. 如果没有可用能力，不要伪称已改名；把 `建议聊天名：<标题>` 作为回复的最后一句，供用户一眼看到并手动采用。
5. 若工具调用失败，报告失败并给出建议标题，不要反复重试；建议标题仍必须是回复最后一句。

## 回复格式

当无法确认已自动改名时，回复最后一行必须只包含：

`建议聊天名：<标题>`

不要在这行后面再添加备注、签名、总结或 emoji。
